...
resampleTo100Hz: async (x, fsIn) => {
  ...
  if (fsIn === 500) return await resample500To100FIR(Float64Array.from(x));
  return downsampleTo(x, fsIn, 100);
},
    // Async MNE FIR bandpass (exact taps from MNE JSON)
    bandpass_04_30_async: (x, fs) => bandpassMNE_04_30_zeroPhase_fs100_async(x, fs),
